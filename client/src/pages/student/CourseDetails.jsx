...
<div className="flex items-center space-x-2 pt-3 pb-1 text-sm">
        ...
        <p className='text-sm'>Course by <span className='text-blue-600 underline'>Nishant Mishra</span></p>
        ...
        <div className='pt-8 text-gray-800'>
          ...
          {courseData.courseContent.map((chapter, index)=>(
           ...
           <div className='flex items-center justify-between px-4 py-3 coursor-pointer select-none' 
           onClick={()=> toggleSection(index)}>
            <div className='flex items-center gap-2'>
              <img className={`transform transition-transform ${openSections[index] 
               ? 'rotate-180' :'' }`}
               src={assets.down_arrow_icon} alt="arrow-icon" />
              <p className='font-medium mf:text-base text-sm'>{chapter.chapterTitle}</p>
            </div>
